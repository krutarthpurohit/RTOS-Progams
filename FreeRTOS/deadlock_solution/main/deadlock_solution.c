 ...
 * Task 1 has released mutex 1
 * Task 2 has released mutex 2
 * Task 1 has released mutex 2
 * Task 2 has released mutex 1
 * Task 1 has acquired mutex 1
 * Task 2 has acquired mutex 2
 * Task 1 has released mutex 1
 * Task 2 has released mutex 2
...
void task1(void *pvParameters) 
{
    while(1) 
    {
        if (xSemaphoreTake(xMutex_1, (TickType_t)10) == pdTRUE)             //Take mutex-1 
        {
            ...
            /* Here we wait for 5 seconds to aquire mutex-2, once it gets time-out we will release the mutex-1 */
            xSemaphoreTake(xMutex_2, (TickType_t)500);         //release mutex-1 if it has mutex-2, wait upto 5 seconds
            {
                /* code */
                xSemaphoreGive(xMutex_1);
                printf("Task 1 has released mutex 1\n");
            }
            vTaskDelay(100);
            xSemaphoreGive(xMutex_2);
            printf("Task 1 has released mutex 2\n");
...
void task2(void *pvParameters) {
    while(1) 
    {
        if (xSemaphoreTake(xMutex_2, (TickType_t)10) == pdTRUE)             //Take mutex-2
        {
            ...
            // Do critical section work here
            vTaskDelay(pdMS_TO_TICKS(1000)); // Simulate some work
            /* Here we wait for 5 seconds to aquire mutex-1, once it gets time-out we will release the mutex-2 */
            xSemaphoreTake(xMutex_1, (TickType_t)500);         //release mutex-2 if it has mutex-1, wait upto 5 seconds
            {
                /* code */
                xSemaphoreGive(xMutex_2);
                printf("Task 2 has released mutex 2\n");
            }
            vTaskDelay(100);
            xSemaphoreGive(xMutex_1);
            printf("Task 2 has released mutex 1\n");
